...
app.post("/api/events", (req, res) => {
  const { title, date, time, end_time, person, note, type, location } =
    req.body;
  const result = db
    .prepare(
      `
    INSERT INTO events (title, date, time, end_time, person, note, type, location)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `,
    )
    .run(
...
app.patch("/api/events/:id", (req, res) => {
  const { title, date, time, end_time, person, type, note, location } =
    req.body;
  db.prepare(
    `UPDATE events SET title=?, date=?, time=?, end_time=?, person=?, type=?, note=?, location=? WHERE id=?`,
  ).run(
    title,
    date,
    time || null,
...
app.get("/api/tasks", (req, res) => {
  const tasks = db
    .prepare("SELECT * FROM tasks WHERE done = 0 ORDER BY created_at DESC")
    .all();
  res.json(tasks);
});
...
app.patch("/api/tasks/:id", (req, res) => {
  const { title, person, due_date, note } = req.body;
  db.prepare(
    `UPDATE tasks SET title=?, person=?, due_date=?, note=? WHERE id=?`,
...
app.patch("/api/dispatches/:id", (req, res) => {
  ...
  db.prepare(`UPDATE dispatches SET title=?, note=?, type=? WHERE id=?`).run(
    title,
    note || null,
    type,
    req.params.id,
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `You are HorsePillow, a warm intelligent family planning assistant for the Lee-Dykhuizen family: Jeff (Commander, travels for work), Jenn (The Admiral, has ADHD, works from home), JJ (The Cadet, age 8, loves soccer), Bentley (the dog).

Extract structured data from this family drop:
"${raw}"

Today's date is ${localDate || new Date().toISOString().split("T")[0]}. Tomorrow is the next day. Use exact dates.

Think about the INVISIBLE mental load — what else needs to happen because of this? Who is affected? What should be flagged?
  ...
  "items": [
    {
      ...
      "note": "HorsePillow's smart context — weather check needed? who needs to know? what to pack? ripple effects? or null",
      "category": "activity" | "appointment" | "work" | "fun" | "event",
      "dispatch_type": "task" | "note" | "remember" or null,
      "to_person": "jeff" | "jenn" or null
    }
  ],
  "scout_response": "HorsePillow's warm direct response. 1-2 sentences. Specific. Real. Acknowledge what was caught. Flag the mental load ripple if there is one. Hype Jeff up when he's on top of things."
}`,
          },
        ],
      }),
    ...
    for (const item of parsed.items) {
      if (item.type === "event" && item.date) {
        db.prepare(
          `INSERT INTO events (title, date, time, end_time, person, type, note, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        ).run(
          item.title,
          item.date,
          item.time || null,
    ...
    // Update drop status
    db.prepare(`UPDATE drops SET status='done', extracted=? WHERE id=?`).run(
    ...
    db.prepare(`UPDATE drops SET status='error' WHERE id=?`).run(
      result.lastInsertRowid,
    );
    res.json({
      id: result.lastInsertRowid,
...
app.get("/api/drops", (req, res) => {
  const drops = db
    .prepare("SELECT * FROM drops ORDER BY created_at DESC LIMIT 20")
    .all();
  res.json(drops);
});
...
app.get("/api/fireside", (req, res) => {
  const items = db
    .prepare("SELECT * FROM fireside WHERE done = 0 ORDER BY created_at DESC")
    .all();
  res.json(items);
});
...
app.patch("/api/fireside/:id", (req, res) => {
  ...
  db.prepare(
    `UPDATE fireside SET title=?, note=?, scheduled_date=? WHERE id=?`,
...
app.get("/api/watchlist", (req, res) => {
  const shows = db
    .prepare("SELECT * FROM watchlist ORDER BY status ASC, updated_at DESC")
    .all();
  res.json(shows);
});
...
app.get("/api/pins", (req, res) => {
  const pins = db
    .prepare("SELECT * FROM pins ORDER BY created_at DESC LIMIT 10")
    .all();
  res.json(pins);
});
...
app.post("/api/pins", (req, res) => {
  ...
  const result = db
    .prepare("INSERT INTO pins (text, from_person, to_person) VALUES (?, ?, ?)")
    .run(text, from_person || "jeff", to_person || "jeff");
  res.json({ id: result.lastInsertRowid, ...req.body });
});
...
app.get("/api/meals", (req, res) => {
  const meals = db
    .prepare(
      'SELECT * FROM meals ORDER BY CASE day WHEN "Mon" THEN 1 WHEN "Tue" THEN 2 WHEN "Wed" THEN 3 WHEN "Thu" THEN 4 WHEN "Fri" THEN 5 WHEN "Sat" THEN 6 WHEN "Sun" THEN 7 END',
    )
    .all();
  res.json(meals);
});
...
app.post("/api/meals", (req, res) => {
  ...
  const result = db
    .prepare("INSERT INTO meals (day, name, note) VALUES (?, ?, ?)")
    .run(day, name, note || null);
  res.json({ id: result.lastInsertRowid, ...req.body });
});
...
app.post("/api/meals/suggest", async (req, res) => {
  try {
    const today = new Date().toISOString().split("T")[0];
    const events = db
      .prepare("SELECT * FROM events WHERE date >= ? ORDER BY date ASC LIMIT 7")
      .all(today);
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: {
...
Respond ONLY with raw JSON — no markdown, no explanation:
{
  ...
  "message": "One warm sentence from HorsePillow about this week's plan"
    ...
    const insert = db.prepare(
      "INSERT INTO meals (day, name, note) VALUES (?, ?, ?)",
    );
    ...
    res.json({
      message: "Could not generate suggestions right now. Try again.",
    });
  }
});
...
app.get("/api/food", (req, res) => {
  const places = db
    .prepare("SELECT * FROM food_places WHERE active = 1 ORDER BY name ASC")
    .all();
  res.json(places);
});
...
app.post("/api/food", (req, res) => {
  ...
  const result = db
    .prepare("INSERT INTO food_places (name, type) VALUES (?, ?)")
    .run(name, type || "restaurant");
  res.json({ id: result.lastInsertRowid, ...req.body });
});

app.delete("/api/food/:id", (req, res) => {
  db.prepare("DELETE FROM food_places WHERE id=?").run(req.params.id);
  res.json({ success: true });
});
...
app.post("/api/grocery", async (req, res) => {
  try {
    const meals = db
      .prepare("SELECT * FROM meals ORDER BY created_at ASC")
      .all();
    if (!meals.length) return res.json({ items: [] });
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `You are HorsePillow. Generate a grocery list for Fry's supermarket based on this week's meal plan for a family of 3 (two adults, one 8-year-old boy) plus a dog named Bentley.

Meal plan: ${meals.map((m) => `${m.day}: ${m.name}${m.note ? " (" + m.note + ")" : ""}`).join(", ")}

Include staples the family likely needs too. Keep it practical and complete.
...
app.get("/api/scout", (req, res) => {
  ...
  const pendingDispatches = db
    .prepare(
      "SELECT COUNT(*) as count FROM dispatches WHERE to_person='jeff' AND done=0",
    )
    .get();
  const pendingTasks = db
    .prepare(
      "SELECT COUNT(*) as count FROM tasks WHERE person='jeff' AND done=0",
    )
    .get();
